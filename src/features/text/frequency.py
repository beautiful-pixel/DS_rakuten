...
class TokenFrequencyTransformer(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, tokens, normalize=False, name_prefix=None):
        """
    ...
    def transform(self, X):
        ...
        X = X.fillna("")
        counts = pd.DataFrame(
            {self.prefix + t: X.str.count(t) for t in self.tokens}
        )
        if self.normalize:
            counts = counts.div(X.apply(len), axis=0).fillna(0)
        return counts